...
// snapshotClients returns a copy of the list of currently connected client
// pointers together with the number of active channels. The server lock is only
// held while copying the map, so the (potentially locking) client getters can be
// called safely by the caller without holding the server lock.
func snapshotClients() ([]*Client, int) {
	sl.Lock()
	defer sl.Unlock()
