...
> **Reference repository.** The official repository for this Accessolutions fork is [accessolutions/NVDA-Remote-GO](https://github.com/accessolutions/NVDA-Remote-GO), branch `main`. All development happens there. The repository [accessolutions/NVDA-Remote-GO-Server](https://github.com/accessolutions/NVDA-Remote-GO-Server) is a read only public mirror, regenerated from `main` without the administration dashboard, the geolocation and the Accessolutions deployment scripts. Do not commit to the mirror directly. The original upstream project is [tech10/nvdaRemoteServer](https://github.com/tech10/nvdaRemoteServer).
...
```console
$ nvdaRemoteServer [-pid-file /path/to/pid/file] [-conf-file /path/to/configuration/file] [-conf-read=true] [-gen-conf-file /path/to/generated/configuration/file] [-gen-conf-dir=false] [-create=false] [-address :6837] [-ws-address :443] [-ws-path /] [-ws-raw=true] [-sni-route turn.example.com=127.0.0.1:5349] [-cert-file /path/to/ssl/certificate] [-key-file /path/to/ssl/key] [-gen-cert-file /path/to/created/cert/file] [-motd "Example message of the day."] [-motd-always-display=false] [-send-origin=true] [-screen-share=false] [-turn-url stun:turn.example.com:3478] [-turn-secret secret] [-turn-secret-file /path/to/turn/secret] [-turn-ttl 3600] [-log-level=0] [-log-file /path/to/log/file] [-admin=false] [-admin-path /admin] [-admin-data-file /data/admin-history.db] [-geoip-api-url https://ipwho.is] [-launch=true]
```
...
Address the server will listen on for secure WebSocket (wss) connections, in the same ip:port form as `-address`, such as `:443`. This lets clients connect through corporate HTTP proxies that only allow HTTPS traffic. It can be declared more than once. If it is unset, no WebSocket listener is started.

By default, these addresses also accept the historic newline delimited NVDA Remote protocol, so a single port serves both the older and the newer clients. See `-ws-raw` below.


#### `-ws-path`

URL path on which the server accepts WebSocket connections, such as `/` or `/ws`. The default is `/`.


#### `-ws-raw`

Also accept the historic newline delimited NVDA Remote protocol on the `-ws-address` addresses. It defaults to true.

Once the TLS handshake is complete, the server looks at the first decrypted byte. The historic protocol is a stream of JSON objects, so it always starts with an opening brace, while a WebSocket client always starts with an HTTP request line. The two are never ambiguous, so both protocols can share the same address and the same certificate.

This is what allows older clients configured on port 443 to keep connecting without being reconfigured. Set this parameter to false to restore the WebSocket only behavior. It has no effect on the `-address` listeners, which always speak the historic protocol.

A full description of the design, the tests and the rollback procedure is available in [TCP-and-WebSocket.md](TCP-and-WebSocket.md).


#### `-sni-route`

Forward the TLS connections asking for a given server name to another TCP endpoint, in the format `name=host:port`, such as `turn.example.com=127.0.0.1:5349`. It can be declared more than once. If it is unset, nothing is routed and the server behaves exactly as before.

Corporate networks routinely allow port 443 and block everything else. The TURN server used by screen sharing then becomes unreachable on its own ports, and screen sharing fails as soon as the two machines are not on the same network, even though signaling works. This parameter lets a TURN server share port 443 with this server.

The server name is read from the ClientHello **before** the TLS handshake takes place, and matching connections are forwarded untouched, so the target service performs its own handshake and presents its own certificate. Connections asking for any other name, **including those that send no name at all**, are served normally: older clients are unaffected.

This is not the same mechanism as `-ws-raw`, which distinguishes protocols *after* the handshake. Both can be used together.

Routing inside the server, rather than putting a TCP proxy in front of it, keeps the real source address of every client, which the administration history and the geolocation depend on.

Example, with coturn listening on 5349 and its own name in the certificate:

```
-ws-address :443
-sni-route turn.example.com=127.0.0.1:5349
-screen-share
-turn-url turns:turn.example.com:443?transport=tcp
-turn-secret-file /data/turn.secret
```
...
Enable relaying of WebRTC screen sharing signaling between clients. This is disabled by default, so upgrading the server changes nothing for existing deployments.

When enabled, clients may advertise their supported features through a `capabilities` message and exchange session descriptions and ICE candidates through the server. Only signaling transits, which amounts to a few kilobytes per session. **Audio and video never pass through the server**, they travel directly between the two clients, or through a TURN relay when a direct connection cannot be established.

Signaling messages are routed to a single target client rather than broadcast, and are refused when the sender or the target is a master that has not been authorized on a locked channel. A rate limit is applied per client.

Clients that do not advertise the screen sharing capability never receive these messages, so older clients are unaffected.


#### `-turn-url`

A STUN or TURN URL handed to clients that ask for ICE servers, such as `stun:turn.example.com:3478` or `turn:turn.example.com:3478?transport=udp`. You may declare this parameter more than once. It requires `-screen-share`.

Without a TURN relay, clients located behind restrictive firewalls will fail to establish a direct connection.


#### `-turn-secret`

The shared secret used to derive ephemeral TURN credentials. It must match the `static-auth-secret` of your TURN server, such as coturn, configured with `use-auth-secret`.

The secret itself is never sent to clients. Each client receives a username and password that expire, which prevents your TURN server from being used as an open relay.

Prefer setting this through a configuration file, since command line parameters are visible to other users of the machine through the process list.


#### `-turn-secret-file`

Read the shared TURN secret from a file instead of passing it on the command line or storing it in the configuration file. Leading and trailing whitespace is stripped.

This is the recommended way of providing the secret. The file is read once at startup, and the server refuses to start if the file is missing or empty. Restrict its permissions so that only the account running the server can read it.

When both `-turn-secret` and `-turn-secret-file` are given, the file takes precedence.


#### `-turn-ttl`

The lifetime in seconds of the ephemeral TURN credentials handed to clients. It defaults to 3600.
...
Enable the web administration dashboard. The dashboard is served by the secure WebSocket listener and is available at `/admin` by default. The fixed username is `administrator`; the password is defined during the first connection and can be persisted with `-admin-password-file`.


#### `-admin-path`

Change the URL path of the web administration dashboard. The default path is `/admin`.


#### `-admin-data-file`

Path to the SQLite database used by the administration history. The database
contains the connection sessions and periodic resource measurements. If this
parameter is empty, the server stores `admin-history.db` next to the admin
password file, or in the current directory when no password file is used.
Use a persistent volume when running in Docker. There is no automatic data
retention limit.

The dashboard's **Historique** link provides three views: daily, monthly and
yearly. Daily rows open a details page with hourly statistics, connection
start/end times, durations, IP addresses, roles and channels. CPU metrics show
both the global machine usage and the usage of the Go server process. Memory
metrics show both global system memory and the Go process allocation.

All history periods use the `Europe/Paris` timezone. The number of unique
connections is based on distinct IP addresses observed during the period.


#### `-geoip-api-url`

Configure the base URL of the IP geolocation API used by the administration dashboard. The default is `https://ipwho.is`. For each public client IP, the dashboard can display the city, region and country in one `Localisation` column. Lookups are asynchronous and cached for 24 hours, so they do not delay client connections and repeated connections do not consume one request each.

Set this parameter to an empty value to disable geolocation. Private, local and invalid IP addresses are never sent to the API. IP geolocation is approximate and must not be considered an exact physical address.
